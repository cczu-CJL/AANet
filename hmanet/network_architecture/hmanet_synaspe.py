...
class DecoderConv_AttenBlock(nn.Module):
    def __init__(self,window_size, in_channels, out_channels, skip_channels=0, head=1, depths=1,spatial_kernel=3, topk=1):
        super().__init__()
...
class BiLevelRoutingAttention(nn.Module):
    ...
    def forward(self, x_in, x_g_in):
        ...
        pad_r = (self.window_size - W_in % self.window_size) % self.window_size
        pad_b = (self.window_size - H_in % self.window_size) % self.window_size
        pad_g = (self.window_size - S_in % self.window_size) % self.window_size
        shortcut_x = F.pad(x_in, (0, pad_r, 0, pad_b, 0, pad_g))
        _, _, S, H, W = shortcut_x.size()

        shortcut_x = shortcut_x.permute(0, 2, 3, 4, 1)
        shortcut_x = rearrange(shortcut_x, 'b (p1 Ms) (p2 Mh) (p3 Mw) c-> b (p1 p2 p3) Ms Mh Mw c', Ms=self.window_size, Mh=self.window_size, Mw=self.window_size, c=C)

        gN, gC, gS, gH, gW = x_g_in.size()
        x_g = rearrange(x_g_in, 'b c Ms Mh Mw  -> b (Ms Mh Mw) c')
        ...
        shortcut_x_g = rearrange(x_g, 'b (p1 Ms) (p2 Mh) (p3 Mw) c -> b (p1 p2 p3) Ms Mh Mw c', p1=S // self.window_size, p2=H // self.window_size, p3=W // self.window_size)
        ...
        out = rearrange(out, '(b nw) m w3 c -> b nw w3 (m c)', nw = (S*H*W) // (self.window_size*self.window_size*self.window_size))

        l_out = out[:,:,:self.window_size * self.window_size * self.window_size,:]

        l_out = self.wo(l_out)
        l_out = l_out + shortcut_l

        l_out = rearrange(l_out, 'b (p1 p2 p3) (Ms Mh Mw) c -> b (p1 Ms) (p2 Mh) (p3 Mw) c',
                      p1=S // self.window_size, p2=H // self.window_size, p3=W // self.window_size,
                      Ms=self.window_size, Mh=self.window_size, Mw=self.window_size)
...
class Conv_Atten_inverse_Layer(nn.Module):
    def __init__(self, dim, head, window_size, spatial_kernel, topk):
        ...
        self.down = nn.Sequential(
            nn.Conv3d(dim, dim, (2, 2, 2), (2, 2, 2), bias=False),
            nn.InstanceNorm3d(dim, eps=1e-5, affine=True),
        )
        self.up = nn.Sequential(
            nn.ConvTranspose3d(dim, dim, 2, 2),
            nn.InstanceNorm3d(dim, eps=1e-5, affine=True))
...
class hmanet(SegmentationNetwork):
    def __init__(self, crop_size=[64, 128, 128], embedding_dim=48, input_channels=1, num_classes=13, conv_op=nn.Conv3d,
                 depths=(2, 2, 2, 2), num_heads=(2, 4, 8, 16), patch_size=(2, 4, 4),
        ...
        self.down1 = BasicResBlock(embed_dim * 2, embed_dim * 2, 3, 2, 1, True)

        self.encoder1 = nn.Sequential(
            *([Conv_Atten_inverse_Layer(embed_dim * 2, 2, window_size=4, spatial_kernel=9, topk=8) for _ in range(depths[0])]))

        self.down2 = BasicResBlock(embed_dim * 2, embed_dim * 4, 3, 2, 1, True)
        self.encoder2 = nn.Sequential(
            *([Conv_Atten_inverse_Layer(embed_dim * 4, 4, window_size=4, spatial_kernel=7, topk=4) for _ in range(depths[1])]))

        self.down3 = BasicResBlock(embed_dim * 4, embed_dim * 8, 3, 2, 1, True)
        self.encoder3 = nn.Sequential(
            *([Conv_Atten_inverse_Layer(embed_dim * 8, 8, window_size=8, spatial_kernel=5, topk=1) for _ in range(depths[2])]))


        self.down4 = BasicResBlock(embed_dim * 8, embed_dim * 16, 3, 2, 1, True)
        self.encoder4 = nn.Sequential(
            *([Conv_Atten_inverse_Layer(embed_dim * 16, 16, window_size=4, spatial_kernel=3, topk=1) for _ in range(depths[3])]))

        self.ups0 = DecoderConv_AttenBlock(8, embed_dim * 16, embed_dim * 8, embed_dim * 8, head=8, depths=depths[2], spatial_kernel=5, topk=1)
        self.ups1 = DecoderConv_AttenBlock(4, embed_dim * 8, embed_dim * 4, embed_dim * 4, head=4, depths=depths[1], spatial_kernel=7, topk=4)
        self.ups2 = DecoderConv_AttenBlock(4, embed_dim * 4, embed_dim * 2, embed_dim * 2,head=2, depths=depths[0], spatial_kernel=9, topk=8)

        self.ups16 = nn.ConvTranspose3d(embed_dim * 8, num_classes, (2, 4, 4), (2, 4, 4))
        self.ups8 = nn.ConvTranspose3d(embed_dim * 4, num_classes, (2, 4, 4), (2, 4, 4))
        self.ups4 = nn.ConvTranspose3d(embed_dim * 2, num_classes, (2, 4, 4), (2, 4, 4))
...
if __name__ == '__main__':
    img = torch.randn([1, 1, 64, 128, 128])
    model = hmanet()
    out = model(img)
    print(type(out))
    print(len(out))
